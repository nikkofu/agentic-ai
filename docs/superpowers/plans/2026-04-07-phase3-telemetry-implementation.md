...
- [x] **Step 1: 注入 Span 生命周期**
在 `run()` 方法中：
  - 开启 `agent_run` Span。
  - 调用结束后，将 `tokens` 和 `cost` 附加为 Span Attribute。
  - 上报 Metrics (Counter)。

- [x] **Step 2: 编写测试验证 Span 属性**
